 ...
 */
static vector<string> reconstructPath(unordered_map<string, string> cameFrom, string current) {
    vector<string> path;
...
vector<string> aStarSearch(Graph& graph, string start, string goal) {
    ...
    while (!openSet.empty()) {
        ...
        if (gScore.find(current.city) != gScore.end() && current.g > gScore[current.city]) {
            continue;
        }

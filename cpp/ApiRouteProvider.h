...
/*
 * ApiRouteProvider: fetches ONE driving route from OpenRouteService Directions
 * API.
 *
 * Call setQuery() with resolved source/destination coordinates, then
 * fetchRoutes(). Parses ORS segments into a chain of RouteRecord objects for
 * GraphBuilder.
 *
...
private:
  ...
  static std::string
  httpPostJson(const std::string &url, const std::string &jsonBody,
  ...
  static bool
  parseGeoJsonCoordinates(const std::string &json,
...
public:
  ApiRouteProvider(std::string endpointUrl,
                   const CityCoordinateStore *cityStore);

  /*
   * Set the trip to fetch before calling fetchRoutes().
   * Coordinates come from LocationResolver (cache or geocoding).
   */
  void setQuery(std::string sourceName, double sourceLat, double sourceLon,
                std::string destName, double destLat, double destLon);
